...
class Puzzle_2:
  ...
  def setup(self):
    ...
    for camada in ['inicio1', 'fim1', 'linhas', 'colocarforno', 'aguardar', 'tempo', 'sim', 'nao']:   
      for x, y, surf in tmx_data.get_layer_by_name(camada).tiles():
  ...
  def run(self):
    ...
    while running:
      ...
      self.display_message("Este diagrama deve realizar um processo de repetição em que um biscoito é assado a cada ciclo. Essa estrutura precisa repetir um determinado conjunto de ações várias vezes até atender a uma condição final.")
  ...
  def display_message(self, message):
    ...
    for word in words:
      ...
      if word[0].isupper() and current_line:
        lines.append(' '.join(current_line))
        current_line = [word]
      elif sum(self.font.size(w)[0] for w in current_line) + space_width * len(current_line) + word_width <= max_width:
