...
class Puzzle_1:
  ...
  def setup(self):
    tmx_data = load_pygame('./data/puzzle nivel1.tmx')
    ...
    for camada in ['inicio1', 'fim1', 'linha1', 'soma2']:
      for x, y, surf in tmx_data.get_layer_by_name(camada).tiles():
        Generic((x * TAMANHO, y * TAMANHO), surf, [self.all_sprites], CAMADAS['puzzle'])

    for camada in ['encpeca AB1', 'encpeca AB2', 'encpeca C']:
      for x, y, surf in tmx_data.get_layer_by_name(camada).tiles():
        encaixes = EncPeca((x * TAMANHO, y * TAMANHO), surf, [self.all_sprites], camada.replace('2', '1'), CAMADAS['base_puzzle'])
        self.todos_encaixes.append(encaixes)

    for camada in self.items:
      if camada.nome in ['peca AB1', 'peca AB2', 'peca C']:
